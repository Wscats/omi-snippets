!function() {
    ...
    function getUse(data, paths) {
        ...
        paths.forEach(function(path, index) {
            const isPath = 'string' === typeof path;
    ...
    function getTargetByPath(origin, path) {
        let arr = path.replace(/]/g, '').replace(/\[/g, '.').split('.');
        let current = origin;
        for (let i = 0, len = arr.length; i < len; i++) current = current[arr[i]];
        return current;
    }
    ...
    function define(name, ctor) {
        if ('WeElement' === ctor.is) {
            ...
            const Element = function(_WeElement) {
                function Element() {
                    var _temp, _this, _ret;
                    _classCallCheck(this, Element);
                    for (const _len = arguments.length, args = Array(_len), key = 0; key < _len; key++) args[key] = arguments[key];
                ...
                Element.prototype.use = function(option) {
                    let _this2 = this;
                    this.C++;
                    const updater = function updater(newValue) {
                        const item = _this2.D[updater.id];
                        item.data = newValue;
                        _this2.update();
    ...
    function matchGlobalData(globalData, diffResult) {
        ...
        for (var keyA in diffResult) {
            ...
            for (let i = 0, len = globalData.length; i < len; i++) if (includePath(keyA, globalData[i])) return !0;
        }
        return !1;
    }
    ...
    function getArrayPatch(path, store) {
        ...
        for (let i = 1, len = arr.length; i < len - 1; i++) current = current[arr[i]];
        return {
            k: fixArrPath(path),
            v: current
        };
    ...
    const JSONPatcherProxy = function() {
        ...
        function setTrap(instance, target, key, newValue) {
            let parentPath = findObjectPath(instance, target);
            let destinationPropKey = parentPath + '/' + escapePathComponent(key);
            ...
            if (void 0 === newValue) {
                ...
                let oldValue = instance.proxifiedObjectsMap.get(target[key]);
                if (oldValue) {
            ...
            let reflectionResult = Reflect.set(target, key, newValue);
        ...
        JSONPatcherProxy.prototype.proxifyObjectTree = function(root) {
            ...
            const proxifiedObject = this.A(void 0, root, '');
            this.isProxifyingTreeNow = !1;
            this.resume();
    ...
    const WeElement = function(_HTMLElement) {
        function WeElement() {
            ...
            const _this = _possibleConstructorReturn$1(this, _HTMLElement.call(this));
        ...
        WeElement.prototype.disconnectedCallback = function() {
            this.uninstall();
            this.B = !1;
            if (this.store) for (let i = 0, len = this.store.instances.length; i < len; i++) if (this.store.instances[i] === this) {
    ...
    const ModelView = function(_WeElement) {
        function ModelView() {
    ...
    const n = function(t, r, u, e) {
        for (let p = 1; p < r.length; p++) {
            let s = r[p++], a = "number" === typeof s ? u[s] : s;
        ...
        for (var t, r, u = 1, e = "", p = "", s = [ 0 ], a = function(n) {
            ...
            f && (1 === u && a(), a(f));
            for (let h = 0; h < n[f].length; h++) t = n[f][h], 1 === u ? "<" === t ? (a(), s = [ s ], u = 3) : e += t : p ? t === p ? p = "" : e += t : '"' === t || "'" === t ? p = t : ">" === t ? (a(), 
        ...
        for (const r = "", e = 0; e < n.length; e++) r += n[e].length + "-" + n[e];
        return u[r] || (u[r] = t(n));
    };
